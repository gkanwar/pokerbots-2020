...
class Player(Bot):
    ...
    def get_action(self, game_state, round_state, active):
        ...
        if street == 0:
            ...
            MDF_FACTOR = 1.0 # slightly tighter post


        if street == 0: # PRE = equity
            if my_pip == 1: # open 85% of our button preflop
                pot_odds = 0.4
                raise_odds = 0.45
            else:
            ...
            if equity > raise_odds and opp_contribution < STARTING_STACK:
                ...
                return FoldAction()
        else: # POST = not broken
            if self.range is None:
                new_range = []
                for hand, hand_trans in zip(HANDS, self.trans_hands):
                    hand_trans_e7 = HANDS_E7[hand_trans]
            ...
            #start = time.time()
            if street > 3 and street != self.sorted_street:
                hand_values = {}
